...
class GridField extends FormField {
	 ...
	 * @var string
	 */
	protected $emptyBodyTemplate = 'GridField_EmptyBody';
	/**
	 * @var string
	 */
	protected $name = '';
	...
	/**
	 * Set the template name for rendering the table body if no records are found
	 * @param string $template
	 * @return GridField $this
	 */
	public function setEmptyBodyTemplate($template) {
		$this->emptyBodyTemplate = $template;
		return $this;
	}

	/**
	 * Get the template name for rendering the table body if no records are found
	 * @return string
	 */
	public function getEmptyBodyTemplate() {
	...
	public function FieldHolder($properties = array()) {
		...
		if($total > 0) {
			foreach($list as $idx => $record) {
				...
				$body .= $row->renderWith($this->getRowTemplate()) . "\r\n";
			}
		}
		if (!$body) {
			$body = $this->customise(array('Colspan' => count($columns)))->renderWith($this->getEmptyBodyTemplate());
		}
		$Attributes = array();
		foreach($this->getAttributes() as $k => $v)
			$Attributes[] = ArrayData::create(array('Key' => $k, 'Value' => $v));
		$TableAttributes = array();
		foreach($this->getTableAttributes() as $k => $v)
			$TableAttributes[] = ArrayData::create(array('Key' => $k, 'Value' => $v));
		return $this->customise(array(
			'HTMLFragments' => ArrayData::create($this->getHTMLFragments()),
			'Attributes' => ArrayList::create($Attributes),
			'TableAttributes' => ArrayList::create($TableAttributes),
			'TableBody' => $body,
	...
	/**
	 * Generate GridField HTMLFragments (before, after, header, footer) from all Components
	 * @return array
	 */
