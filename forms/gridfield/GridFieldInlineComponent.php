 ...
 */
abstract class GridFieldInlineComponent implements GridFieldComponent {
	/**
	 * methods and properties replacing a method
	 * @var array
	 */
	...
	public $class;
	/**
	 * @param mixed $methods,... callbacks or properties (that replace a method) to be set on this Component
	 * @return GridFieldInlineComponent
	 */
	...
	public function __construct() {
		...
		$args = func_get_args();
		$args = is_array($args[0]) ? $args[0] : func_get_args();
		foreach ($args as $k => $v)
			$this->methods[$this->stripMethodName($k)] = $v;
		$class = new ReflectionClass($this->class);
		foreach ($class->getInterfaces() as $name => $i) {
			foreach ($i->getMethods() as $m) {
				$m = $this->stripMethodName($m->name);
				if (!isset($this->methods[$m]))
					user_error("Method '$m' is not set, but required by the interface '$name'");
			}
		}
	...
	/**
	 * Helper method to call a method stored in $this->methods
	 * @param $method
	 * @param $args
	 * @return mixed
	 */
	protected function callMethod($method, $args = array()) {
		$args[] = $this;
		$method = $this->methods[$this->stripMethodName($method)];
		return is_callable($method) ? call_user_func_array($method, $args) : $method;
	}
	/**
	 * Strip $name from get prefix
	 * @param string $name
	 * @return string
	 */
	protected function stripMethodName($name) {
		return strtolower(str_replace('get', '', $name));
	}
	/**
	 * @param string $method
	 * @return bool
	...
	public function hasMethod($method) {
		return method_exists($this, $method) || isset($this->methods[strtolower($method)]);
	}
	/**
	 * @param string $method
	...
	public function __call($method, $arguments) {
		if (isset($this->methods[$this->stripMethodName($method)]))
 ...
 */
class GridFieldInlineComponent_HTMLProvider extends GridFieldInlineComponent implements GridField_HTMLProvider {
	/**
 ...
 */
class GridFieldInlineComponent_ColumnProvider extends GridFieldInlineComponent implements GridField_ColumnProvider {
	/**
 ...
 */
class GridFieldInlineComponent_ActionProvider extends GridFieldInlineComponent implements GridField_ActionProvider {
	/**
 ...
 */
class GridFieldInlineComponent_DataManipulator extends GridFieldInlineComponent implements GridField_DataManipulator {
	/**
 ...
 */
class GridFieldInlineComponent_URLHandler extends GridFieldInlineComponent implements GridField_URLHandler {
	/**
